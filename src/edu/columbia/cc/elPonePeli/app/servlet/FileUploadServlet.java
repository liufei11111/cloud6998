...
//        request.getP
        ...
        System.out
                .println("Calling video store method to perform upload to S3 ...");
        VideoStore store = new VideoStore()
                .withCredentials(new BasicAWSCredentials(
                        AwsCredentialConstants.ACCESS.getValue(),
                        AwsCredentialConstants.SECRET.getValue()));
        Video video = store.storeIntoBucket(filename, filecontent);
        System.out.println("Call finished.");

        if (video == null) {
            System.out
                    .println("Some error occurred. No details to persist in the database.");
        } else {
            ...
            transcoder.init();
            transcoder.helpTranscode(video.getId(), video.getVideoName(),
                    video.getBucketName(), video.getBucketName());
        }
